...
router.get('/posts',async function(req,res){
   ...
   let order=req.query.order;
    ...
    posts.sort(function(resA,resB){
        if(order =='asc' && resA._id > resB._id){
            return -1;
        } else if(order =='desc' && resB._id > resA._id){
            return 1;
        }
        return -1;
    })
    res.render('post-lists',{posts:posts,nextOrder:nextOrder});
});
...
router.get('/posts/:id',async function(req,res){
    ...
    const comments=await db.getDb().collection('comments').findOne({_id:new ObjectId(postId)});
    if(!post){
...
router.get('/posts/:id/comments',async function(req,res){
    ...
    const comments=await db.getDb().collection('comments').findOne({postId:postId});
   res.json(comments);
});
